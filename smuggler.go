...
func SmugglerGetRegion(data *DuneMetadata, i uint) byte {
	return ((*data).modified)[(data.smugglersOffset+(i-1)*(SMUGGLER_SIZE+SMUGGLER_PADDING))+0]
}

func SmugglerSetRegion(data *DuneMetadata, i uint, value byte) {
	((*data).modified)[(data.smugglersOffset+(i-1)*(SMUGGLER_SIZE+SMUGGLER_PADDING))+0] = value
}

func SmugglerGetWillingnessToHaggle(data *DuneMetadata, i uint) byte {
	return ((*data).modified)[(data.smugglersOffset+(i-1)*(SMUGGLER_SIZE+SMUGGLER_PADDING))+1]
}

func SmugglerSetWillingnessToHaggle(data *DuneMetadata, i uint, value byte) {
	((*data).modified)[(data.smugglersOffset+(i-1)*(SMUGGLER_SIZE+SMUGGLER_PADDING))+1] = value
}

func SmugglerGetFieldC(data *DuneMetadata, i uint) byte {
	return ((*data).modified)[(data.smugglersOffset+(i-1)*(SMUGGLER_SIZE+SMUGGLER_PADDING))+2]
}

func SmugglerSetFieldC(data *DuneMetadata, i uint, value byte) {
	((*data).modified)[(data.smugglersOffset+(i-1)*(SMUGGLER_SIZE+SMUGGLER_PADDING))+2] = value
}

func SmugglerGetFieldD(data *DuneMetadata, i uint) byte {
	return ((*data).modified)[(data.smugglersOffset+(i-1)*(SMUGGLER_SIZE+SMUGGLER_PADDING))+3]
}

func SmugglerSetFieldD(data *DuneMetadata, i uint, value byte) {
	((*data).modified)[(data.smugglersOffset+(i-1)*(SMUGGLER_SIZE+SMUGGLER_PADDING))+3] = value
}

func SmugglerGetHarvesters(data *DuneMetadata, i uint) byte {
	return ((*data).modified)[(data.smugglersOffset+(i-1)*(SMUGGLER_SIZE+SMUGGLER_PADDING))+4]
}

func SmugglerSetHarvesters(data *DuneMetadata, i uint, value byte) {
	((*data).modified)[(data.smugglersOffset+(i-1)*(SMUGGLER_SIZE+SMUGGLER_PADDING))+4] = value
}

func SmugglerGetOrnithopters(data *DuneMetadata, i uint) byte {
	return ((*data).modified)[(data.smugglersOffset+(i-1)*(SMUGGLER_SIZE+SMUGGLER_PADDING))+5]
}

func SmugglerSetOrnithopters(data *DuneMetadata, i uint, value byte) {
	((*data).modified)[(data.smugglersOffset+(i-1)*(SMUGGLER_SIZE+SMUGGLER_PADDING))+5] = value
}

func SmugglerGetKrysKnives(data *DuneMetadata, i uint) byte {
	return ((*data).modified)[(data.smugglersOffset+(i-1)*(SMUGGLER_SIZE+SMUGGLER_PADDING))+6]
}

func SmugglerSetKrysKnives(data *DuneMetadata, i uint, value byte) {
	((*data).modified)[(data.smugglersOffset+(i-1)*(SMUGGLER_SIZE+SMUGGLER_PADDING))+6] = value
}

func SmugglerGetLaserGuns(data *DuneMetadata, i uint) byte {
	return ((*data).modified)[(data.smugglersOffset+(i-1)*(SMUGGLER_SIZE+SMUGGLER_PADDING))+7]
}

func SmugglerSetLaserGuns(data *DuneMetadata, i uint, value byte) {
	((*data).modified)[(data.smugglersOffset+(i-1)*(SMUGGLER_SIZE+SMUGGLER_PADDING))+7] = value
}

func SmugglerGetWeirdingModules(data *DuneMetadata, i uint) byte {
	return ((*data).modified)[(data.smugglersOffset+(i-1)*(SMUGGLER_SIZE+SMUGGLER_PADDING))+8]
}

func SmugglerSetWeirdingModules(data *DuneMetadata, i uint, value byte) {
	((*data).modified)[(data.smugglersOffset+(i-1)*(SMUGGLER_SIZE+SMUGGLER_PADDING))+8] = value
}

func SmugglerGetHarvestersPrice(data *DuneMetadata, i uint) uint16 {
	return uint16(((*data).modified)[(data.smugglersOffset+(i-1)*(SMUGGLER_SIZE+SMUGGLER_PADDING))+9]) * 10
}

func SmugglerSetHarvestersPrice(data *DuneMetadata, i uint, value uint16) {
	((*data).modified)[(data.smugglersOffset+(i-1)*(SMUGGLER_SIZE+SMUGGLER_PADDING))+9] = byte(value / 10)
}

func SmugglerGetOrnithoptersPrice(data *DuneMetadata, i uint) uint16 {
	return uint16(((*data).modified)[(data.smugglersOffset+(i-1)*(SMUGGLER_SIZE+SMUGGLER_PADDING))+10]) * 10
}

func SmugglerSetOrnithoptersPrice(data *DuneMetadata, i uint, value uint16) {
	((*data).modified)[(data.smugglersOffset+(i-1)*(SMUGGLER_SIZE+SMUGGLER_PADDING))+10] = byte(value / 10)
}

func SmugglerGetKrysKnivesPrice(data *DuneMetadata, i uint) uint16 {
	return uint16(((*data).modified)[(data.smugglersOffset+(i-1)*(SMUGGLER_SIZE+SMUGGLER_PADDING))+11]) * 10
}

func SmugglerSetKrysKnivesPrice(data *DuneMetadata, i uint, value uint16) {
	((*data).modified)[(data.smugglersOffset+(i-1)*(SMUGGLER_SIZE+SMUGGLER_PADDING))+11] = byte(value / 10)
}

func SmugglerGetLaserGunsPrice(data *DuneMetadata, i uint) uint16 {
	return uint16(((*data).modified)[(data.smugglersOffset+(i-1)*(SMUGGLER_SIZE+SMUGGLER_PADDING))+12]) * 10
}

func SmugglerSetLaserGunsPrice(data *DuneMetadata, i uint, value uint16) {
	((*data).modified)[(data.smugglersOffset+(i-1)*(SMUGGLER_SIZE+SMUGGLER_PADDING))+12] = byte(value / 10)
}

func SmugglerGetWeirdingModulesPrice(data *DuneMetadata, i uint) uint16 {
	return uint16(((*data).modified)[(data.smugglersOffset+(i-1)*(SMUGGLER_SIZE+SMUGGLER_PADDING))+13]) * 10
}

func SmugglerSetWeirdingModulesPrice(data *DuneMetadata, i uint, value uint16) {
	((*data).modified)[(data.smugglersOffset+(i-1)*(SMUGGLER_SIZE+SMUGGLER_PADDING))+13] = byte(value / 10)
}

func SmugglerPrint(data *DuneMetadata, i uint) {
	// Number smugglers from 1
	if i > 0 && i <= SMUGGLER_MAX_ID {
